---
name: hooks-dev
description: '处理 nil-design 仓库中 @nild/hooks React hooks 的设计、开发、重构、测试、性能基准、文档化或维护，包括公开导出、DOM target 解析、订阅/observer/timer 清理、SSR/浏览器边界、hooks 文档和行为回归测试。'
---
...
- 以 `packages/hooks/src/index.ts` 的公开 hooks 为覆盖清单；每个 hook 在相邻 `__benchmarks__/` 放一份 `<hook-name>.bench.ts`，新增或移除公开 hook 时同步维护。
- 使用 `renderHook` 测量关键生命周期；按公开行为覆盖 update、callback、订阅绑定、事件分发或 cleanup，不只测空挂载。
- DOM 或浏览器 hook 使用 `// @vitest-environment jsdom`；每次采样后清理 timer、RAF、observer、listener、scroll lock 和 storage，并保持 effect 依赖输入的引用稳定。
- 新增或修改用例后运行 `pnpm hooks:bench`。
